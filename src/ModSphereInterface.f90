...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine pack_vars_1blk(index,dir,pole,out_array)
    ...
    real, dimension(-1:nLons+2, -1:nLats+2, nAlts, nIonsAdvect) :: ionsTmp
    ...
    if (UseIonAdvection) then
       ionsTmp = IDensityS(:,:,1:nAlts,1:nIonsAdvect,index)
  ...
  subroutine unpack_vars_1blk(index,dir,in_array)
    ...
    if (UseIonAdvection) then
       call AB_1blk4_gc_unpack(nLons,nLats,nAlts,nIonsAdvect,2, &
            IDensityS(:,:,1:nAlts,1:nIonsAdvect,index),dir,p,in_array)
       if (NonMagnetic) then
          call AB_1blk4_gc_unpack(nLons,nLats,nAlts,3,2, &
